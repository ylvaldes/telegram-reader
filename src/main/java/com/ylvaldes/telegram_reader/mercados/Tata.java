...
import com.ylvaldes.telegram_reader.archivos.Excel;
import com.ylvaldes.telegram_reader.archivos.Txt;
...
public class Tata {
	...
	public void leerDatos(String filename, String output, String mer) {
		logger.debug("Entrada: " + filename);
		...
		logger.info("Moneda: " + lineasPDF.get(7));
		moneda = lineasPDF.get(7);
		logger.debug("Fecha: " + lineasPDF.get(5));

		logger.debug("Direccion: " + lineasPDF.get(16));
		...
		logger.info(lineasPDF.get(postFechaHora + 1).split("   ")[1]);
		try {
		...
		logger.info("TOTAL A PAGAR: " + lineasPDF.get(posTotal).substring(13).trim());
		...
		if (postLey19 > 0) {
			logger.info("Desc. Ley 19210: " + lineasPDF.get(postLey19).substring(29).trim());
		...
		if (totalPagarPDF == totalSuma) {
			logger.info("Información de la Compra es Correcta");
			if (ley > 0) {
	...
	public void procesarCompra(List<String> compras, String datosExtra) {
		...
		for (String string : compras) {
			...
			if (matcherCantP.find()) {
				...
				logger.info("Cantidad de Producto: " + cantidad);
			}
			...
			if (matcherCantP.find()) {
				...
				precioOriginal = precioOriginal.replace(",", ".");
				logger.info("Precio Original: " + precioOriginal);
			}
			...
			if (matcherCantP.find()) {
				...
				precioDescuento = precioDescuento.replace(",", ".");
				logger.info("Precio con Descuento: " + precioDescuento);
			}
